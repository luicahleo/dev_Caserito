...
New-Item -ItemType Directory -Force (Join-Path $raizCaserito '.local/pc3/caddy-data') | Out-Null
New-Item -ItemType Directory -Force (Join-Path $raizCaserito '.local/pc3/caddy-config') | Out-Null
Set-Content -Path (Join-Path $raizCaserito '.local/pc3/ip.txt') -Value $ipLan -Encoding ascii
...
$archivosCompose = @(
    ...
    '-f', 'docker-compose.pc3.yml',
    '-f', 'docker-compose.argos.yml'
)

if ($RecrearDatos) {
    & docker compose @archivosCompose down --volumes
    if ($LASTEXITCODE -ne 0) { throw 'No se pudieron eliminar los datos locales de PC3.' }
...
if ($LASTEXITCODE -ne 0) { throw 'No se pudo levantar el entorno PC3.' }

$certificado = Join-Path $raizCaserito '.local/pc3/caddy-data/caddy/pki/authorities/local/root.crt'
...
Write-Host ''
Write-Host "Caserito PC3: https://$hostLan" -ForegroundColor Green
