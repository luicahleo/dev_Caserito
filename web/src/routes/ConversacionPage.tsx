...
export function ConversacionPage() {
  ...
  const [cursoresRecibos, setCursoresRecibos] = useState({ entregada: 0, leida: 0 });
  ...
  const ultimaSecuenciaAjena =
    mensajes.filter((mensaje) => mensaje.remitenteId !== usuario?.id).at(-1)?.secuencia ?? 0;
  useEffect(() => {
    if (!conversacion || ultimaSecuenciaAjena === 0) return;
    void marcarEntrega(id, ultimaSecuenciaAjena)
      .then(() => marcarLectura(id, ultimaSecuenciaAjena))
      .then(async () => {
        await Promise.all([
      ...
      .catch(() => undefined);
  }, [conversacion, id, ultimaSecuenciaAjena, queryClient]);
  ...
  useEffect(() => {
    ...
    cliente.alActualizarEstado((estado) => {
      if (estado.conversacionId !== id) return;
      setCursoresRecibos((actual) => ({
        entregada: Math.max(actual.entregada, Number(estado.ultimaSecuenciaEntregada)),
        leida: Math.max(actual.leida, Number(estado.ultimaSecuenciaLeida)),
      }));
    });
    void cliente
      .suscribir(id)
      .then(() => setErrorTiempoReal(false))
      .catch(() => setErrorTiempoReal(true));
    return () => {
      void cliente.desuscribir(id).finally(() => cliente.detener());
    };
  }, [cliente, conversacion, id, queryClient]);

  useEffect(() => {
    navigator.serviceWorker?.controller?.postMessage({
      tipo: 'conversacion-visible',
      conversacionId: id,
    });
    return () =>
      navigator.serviceWorker?.controller?.postMessage({
  ...
  if (consultaConversacion.error || historial.error || !conversacion) {
    return (
      <Container maxWidth="sm" sx={{ py: 4 }}>
        ...
        <Button component={RouterLink} to="/mensajes" sx={{ mt: 2 }}>
          Volver a mensajes
        </Button>
      </Container>
    );
  ...
  const secuenciaEntregada = Math.max(
    cursoresRecibos.entregada,
    conversacion.ultimaSecuenciaEntregadaContraparte ?? 0,
  );
  const secuenciaLeida = Math.max(
    cursoresRecibos.leida,
    conversacion.ultimaSecuenciaLeidaContraparte ?? 0,
  );
  return (
    <Container maxWidth="md" sx={{ py: { xs: 1, sm: 3 } }}>
      <Stack
        direction="row"
        sx={{ mb: 2, gap: 1, justifyContent: 'space-between', alignItems: 'center' }}
      >
        <Box>
        ...
        <Chip
          ...
          color={puedeEnviar ? 'success' : 'default'}
        />
      </Stack>
      {errorTiempoReal && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          Conexión en tiempo real interrumpida. Recuperaremos los mensajes al reconectar.
        </Alert>
      )}
      {!puedeEnviar && (
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ mb: 2 }}>
        {cursor && (
          <Button
            onClick={() =>
              void obtenerMensajes(id, cursor).then((pagina) => {
                setMensajesAnteriores((actuales) => combinar(pagina.items, actuales));
                setCursorCargado(pagina.siguienteCursor);
              })
            }
          >
        ...
        <Button color="warning" onClick={() => accion.mutate('bloquear')}>
          Bloquear contraparte
        </Button>
        <Button onClick={() => accion.mutate('desbloquear')}>Desbloquear contraparte</Button>
        <Button color="error" onClick={() => setReporte({ tipo: 1 })}>
          Reportar conversación
        </Button>
        <Button color="error" onClick={() => setReporte({ tipo: 3 })}>
          Reportar contraparte
        </Button>
      </Stack>
      {accion.isError && <Alert severity="error">No se pudo completar la acción.</Alert>}
      <Paper
        variant="outlined"
        sx={{ p: { xs: 1, sm: 2 }, minHeight: 280, maxHeight: '55vh', overflowY: 'auto' }}
      >
        {mensajes.length === 0 && (
          <Typography color="text.secondary">Aún no hay mensajes.</Typography>
        )}
        <Stack spacing={1}>
          {mensajes.map((mensaje) => {
            ...
            return (
              <Box
                key={mensaje.id}
                sx={{ alignSelf: propio ? 'flex-end' : 'flex-start', maxWidth: '85%' }}
              >
                <Paper sx={{ px: 1.5, py: 1, bgcolor: propio ? 'primary.light' : 'grey.100' }}>
                  ...
                  >
                    <Typography variant="caption">
                      {new Date(mensaje.enviadoEn).toLocaleTimeString('es-ES', {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </Typography>
                ...
                <Button
                  size="small"
                  color="error"
                  onClick={() => setReporte({ tipo: 2, mensajeId: mensaje.id })}
                >
                  Reportar mensaje
                </Button>
              </Box>
      ...
      >
        ...
        <Button
          type="submit"
          variant="contained"
          disabled={!puedeEnviar || enviar.isPending || !texto.trim()}
        >
          Enviar
        </Button>
      </Stack>
      {enviar.isError && (
        <Alert severity="error" sx={{ mt: 1 }}>
          No se pudo enviar. Puedes intentarlo nuevamente.
        </Alert>
      )}
      <Dialog open={reporte !== null} onClose={() => setReporte(null)} fullWidth>
        ...
        <DialogActions>
          <Button onClick={() => setReporte(null)}>Cancelar</Button>
          <Button
            variant="contained"
